...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    public GestureLockViewGroup(Context context, AttributeSet attrs,
        ...
        mNoFingerColor = a.getColor(R.styleable.GestureLockViewGroup_color_no_finger, mNoFingerColor);//无手指触摸的状态下圆的颜色
        mFingerOnColor = a.getColor(R.styleable.GestureLockViewGroup_color_finger_on, mFingerOnColor);//手指触摸的状态下圆的颜色
        mFingerUpColorCorrect = a.getColor(R.styleable.GestureLockViewGroup_color_finger_up_correct, mFingerUpColorCorrect);//手指抬起的状态下，正确圆的颜色
        mFingerUpColorError = a.getColor(R.styleable.GestureLockViewGroup_color_finger_up_error, mFingerUpColorError);//手指抬起的状态下，错误圆的颜色
        mCount = a.getInt(R.styleable.GestureLockViewGroup_count, mCount);//每个边上的GestureLockView的个数
        mPrferenceId = a.getInt(R.styleable.GestureLockViewGroup_preference_id, mPrferenceId);//保存手势密码id
            ...
            case MotionEvent.ACTION_UP:
                if (isInPasswordSettingMode) {
                    ...
                    if (mChoose.size() > 0) {
                        ...
                        if (!isCorrect) {
                            if (inputPsws.length() >= 4) {
                                gestureUnmatchedExceedListener.onUnmatchedExceedBoundary(mTryTimes);  //超出重试次数，进入回调
                            } else {
                                gestureUnmatchedExceedListener.onUnmatchedLessFourPoint(inputPsws.length());
                            }
                        }
                    } else {
                        return true;
                  ...
                  /*  if (gestureEventListener != null) {
                    ...
                    //  if (this.mTryTimes == 0) {//修改回调传回剩余可输入次数  2016-9-22 by lion
                /*    if (!isCorrect) {
                        if (mChooseString.length() >= 4) {
                            gestureUnmatchedExceedListener.onUnmatchedExceedBoundary(mTryTimes);  //超出重试次数，进入回调
                        } else {
                            gestureUnmatchedExceedListener.onUnmatchedLessFourPoint(mChooseString.length());
                        }
                    }*/
                }
                drawWhenTouchUp();
 ...
 /*       for (int i = 0; i + 1 < mChoose.size(); i++) {
    ...
    private void setPasswordHandle() {
        if (isWaitForFirstInput) {
            ...
            if (firstInputPassword.equals(mChooseString)) {
                ...
                gesturePasswordSettingListener.onSuccess(mChooseString);
                isInPasswordSettingMode = false;
            } else {
                gesturePasswordSettingListener.onFail();
                isWaitForFirstInput = true; //两次不一样后重新进入密码设置流程
            }
        }
        reset();
    }
     ...
     * @return
     */
private String inputPsws;
    public boolean checkAnswer() {
        inputPsws=mChooseString;
        if (password.equals(mChooseString)) {
            return true;
        } else {
            if (isRetryTimeLimit&&mChooseString.length()>=4)
                this.mTryTimes--;
            return false;
        }
    }
    ...
    public void motifyPasswordReset() {//修改手势密码，保证用户第一次验证手势密码成功后，放弃修改,
        // 之前的仍有效，若继续设置，两次设置完成后才覆盖之前的旧手势密码
        isWaitForFirstInput = true;
        isInPasswordSettingMode = true;
    }
