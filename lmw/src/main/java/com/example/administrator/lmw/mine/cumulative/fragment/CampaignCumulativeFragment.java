...
import com.example.administrator.lmw.mine.cumulative.adapter.CampaignAdapter;
 ...
 */
public class CampaignCumulativeFragment extends BaseFragment implements XListView.IXListViewListener {
                ...
                if (cumulativeEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (cumulativeEntity.getData().getDatas().size() > 0) {
                            adapter = new CampaignAdapter(mContext, cumulativeEntity.getData().getDatas(), R.layout.cumulative_layout_item);
