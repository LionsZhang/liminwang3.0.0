...
import com.example.administrator.lmw.mine.transfer.adapter.TransferProductAdapter;
 ...
 */
public class TransferProductFragment extends BaseFragment implements XListView.IXListViewListener {
    ...
    private void fristhttp() {
        ...
        httpTransfer(pageIndex, pageSize, "1", SharedPreference.getInstance().getString(mContext, PreferenceCongfig.APP_TOKEN, ""));
    }
    ...
    private void headview() {
        ...
        headTv.setText("共计");
        claimsLin.setVisibility(View.GONE);
        financialLin.setVisibility(View.GONE);
                ...
                if (transferCountEntity.getCode().equals("0")) {
                    if (transferCountEntity.getData().getDatas().size() > 0) {
                        ...
                        for (int i = 0; i < transferCountEntity.getData().getDatas().size(); i++) {
                            quantity = transferCountEntity.getData().getDatas().get(i).getRepayCount() + quantity;
                            moneys = transferCountEntity.getData().getDatas().get(i).getRepayAmt() + moneys;
                            if (transferCountEntity.getData().getDatas().get(i).getIsDebtRepay() == 0) {// 理财
                                headQuantity.setText(transferCountEntity.getData().getDatas().get(i).getRepayCount() + "");
                                headMoneys.setText(transferCountEntity.getData().getDatas().get(i).getRepayAmt() + "");
                ...
                if (transferEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (transferEntity.getData().getDatas().size() > 0) {
                            adapter = new TransferProductAdapter(mContext, transferEntity.getData().getDatas(), R.layout.transfer_item);
