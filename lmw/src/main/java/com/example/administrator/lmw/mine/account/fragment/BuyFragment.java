...
import com.example.administrator.lmw.mine.account.adapter.BuyAdapter;
 ...
 */
public class BuyFragment extends BaseFragment implements XListView.IXListViewListener {
    ...
    private int appFundsType = 2;// 类型  1全部,2购买 ,3回款,4充值,5提现,6其他
    ...
    protected void initializeView() {
        xlist();
        getBuy(appFundsType, 1, pageSize);
            ...
            public void run() {
                if (cumulativeXlv != null) {
                    ...
                    if (pageIndex > pageCount) {
                        ...
                        getBuy(appFundsType, pageIndex, pageSize);
                    }
    ...
    private void getBuy(int appFundsType, final int pageIndex, int pageSize) {
                ...
                if (accountEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (accountEntity.getData().getDatas().size() > 0) {
                            adapter = new BuyAdapter(mContext, accountEntity.getData().getDatas(), R.layout.account_item);
                        ...
                        if (accountEntity.getData().getDatas().size() > 0) {
                            adapter.addBuy(accountEntity.getData().getDatas());
