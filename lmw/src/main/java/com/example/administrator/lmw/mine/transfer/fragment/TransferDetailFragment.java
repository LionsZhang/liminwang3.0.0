...
import com.example.administrator.lmw.mine.transfer.adapter.TransferDetailAdapter;
 ...
 */
public class TransferDetailFragment extends BaseFragment implements XListView.IXListViewListener {
    ...
    private void fristhttp() {
        ...
        httpTransfer(pageIndex, pageSize, "0", SharedPreference.getInstance().getString(mContext, PreferenceCongfig.APP_TOKEN, ""));
    }
                ...
                if (transferEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (transferEntity.getData().getDatas().size() > 0) {
                            adapter = new TransferDetailAdapter(mContext, transferEntity.getData().getDatas(), R.layout.transfer_item);
