...
import com.example.administrator.lmw.mine.account.adapter.RechargeAdapter;
...
public class RechargeFragment extends BaseFragment implements XListView.IXListViewListener {
    ...
    private int appFundsType = 4;// 类型  1全部,2购买 ,3回款,4充值,5提现,6其他
    ...
    protected void initializeView() {
        xlist();
        getRecharge(appFundsType, 1, pageSize);
            ...
            public void run() {
                if (cumulativeXlv != null) {
                    ...
                    if (pageIndex > pageCount) {
                        ...
                        getRecharge(appFundsType, pageIndex, pageSize);
                    }
    ...
    private void getRecharge(int appFundsType, final int pageIndex, int pageSize) {
                ...
                if (accountEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (accountEntity.getData().getDatas().size() > 0) {
                            adapter = new RechargeAdapter(mContext, accountEntity.getData().getDatas(), R.layout.account_item);
                        ...
                        if (accountEntity.getData().getDatas().size() > 0) {
                            adapter.addRecharge(accountEntity.getData().getDatas());
