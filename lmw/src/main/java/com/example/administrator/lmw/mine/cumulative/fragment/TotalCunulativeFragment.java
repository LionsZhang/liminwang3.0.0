...
import com.example.administrator.lmw.mine.cumulative.adapter.TotalAdapter;
 ...
 */
public class TotalCunulativeFragment extends BaseFragment implements XListView.IXListViewListener {
                ...
                if (cumulativeEntity.getCode().equals("0")) {
                    if (pageIndex == 1) {
                        ...
                        if (cumulativeEntity.getData().getDatas().size() > 0) {
                            adapter = new TotalAdapter(mContext, cumulativeEntity.getData().getDatas(), R.layout.cumulative_layout_item);
